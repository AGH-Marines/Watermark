...
m = matfile("C:\Users\macie\OneDrive\Pulpit\AGH Marines\Watermark\input\signals\my_signal.mat", "Writable", true);
...
figure;
plot(t(1001:2000), real(x(1001:2000))); % Plots the real part of the signal
title('A segment of the OFDM signal in the time domain');
xlabel('Time [s]');
ylabel('Amplitude [m]');
grid on
...
% Widmo amplitudowo-częstottliwościowe
figure;
plot(f, real(y));
title('Spectrum of the OFDM signal');
xlabel('Frequency [Hz]');
ylabel('Amplitude [m]');
grid on;

% Macierz odpowiedzi impulsowych
% Parametry kanału
tau_max = 0.01; % maksymalne opóźnienie kanału (s)
t_max = length(x)/fs_x; % maksymalny czas trwania
fs_tau = 1e4; % rozdzielczość w opóźnieniu (Hz)
fs_t = 200;   % rozdzielczość w czasie (Hz)
fc = (f_min + f_max) / 2;  % środek pasma (Hz)

% Liczba punktów
N_tau = round(tau_max * fs_tau);  % liczba próbek w opóźnieniu
N_t = ceil(t_max * fs_t);         % liczba punktów w czasie

% Tworzymy sztuczny kanał (np. zanikanie + echo)
h = zeros(N_tau, N_t);
for i = 1:N_t
    delay1 = round(0.002 * fs_tau);  % echo po 2 ms
    delay2 = round(0.005 * fs_tau);  % echo po 5 ms
    h(1, i) = 1;                     % główna ścieżka
    if delay1 <= N_tau, h(delay1, i) = 0.5; end
    if delay2 <= N_tau, h(delay2, i) = 0.3; end
end
